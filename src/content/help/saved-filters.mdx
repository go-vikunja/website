...
slug: saved-filters
description: >-
  Saved filters are personal, cross-project task views built from filter
  queries.
---
import Callout from '../../components/Callout.astro';

Saved filters let you create personal, cross-project task views based on a [filter query](/help/filters). Unlike [views](/help/views), which are tied to a single project and shared with its members, saved filters are private to your account and pull tasks from all your projects at once.
...
- **Filter query**: the [filter expression](/help/filters) that determines which tasks are shown

![Saved filter creation form with title, description, and filter query](../../assets/images/help/saved-filters-create-form.png)

The filter query uses the same syntax as project view filters. You can use all available [filter fields](/help/filters#fields), including the `project` field, which is only available in saved filters and lets you limit results to specific projects.

<Callout type="info">
The default filter for new saved filters is `done = false`, which shows only incomplete tasks.
</Callout>
...
![Saved filters shown in the left sidebar](../../assets/images/help/saved-filters-sidebar.png)
...
You can use a saved filter to customize which tasks appear on your [dashboard](/help/dashboard). To do this:

1. Create a saved filter with the query you want.
2. Open your **[Settings](/help/settings)** and set the filter as the home screen filter.
