...
slug: views
description: >-
  Configure how tasks are displayed: List, Table, Gantt, and Kanban views, each
  with optional filters.
---
import Screenshot from '../../components/Screenshot.astro'


Views control how [tasks](/help/tasks) are shown inside a [project](/help/projects). Every project starts with four default views, and you switch between them using the tabs at the top of the project.

<Screenshot src="views-tabs" alt="View tabs at the top of a project" />

### Which view should I use?

| View | Best for | Key feature |
|------|----------|-------------|
| **[List](#list-view)** | Simple task lists, quick entry | Drag to reorder, [Quick Add Magic](/help/quick-add-magic) |
| **[Table](#table-view)** | Comparing many task attributes | Configurable columns, spreadsheet-like |
| **[Kanban](#kanban-view)** | Workflow stages (To Do / In Progress / Done) | Drag tasks between buckets |
| **[Gantt](#gantt-view)** | Scheduling by date | Timeline with drag-to-reschedule |

You can choose the default project view in your [user settings](/help/settings).
...
Each view can have its own filter. Only matching tasks appear in that view. See [Filters](/help/filters) for the filter syntax.

### Remembered view settings

Vikunja remembers the state of each view separately in your browser. List and table views keep their sort order, filter, search, and current page. Gantt views keep their date range and whether tasks without dates are shown. Reloading the page, restarting the desktop app, or returning through the sidebar restores that view's previous state.

The saved state is local to that browser or desktop app, so it does not follow your account to another device. Query parameters in a bookmarked or shared URL take precedence over the saved state.

## List view

<Screenshot src="views-list" alt="List view showing tasks in a flat list" />
...
- **Quick-add input** at the top lets you create tasks and use [Quick Add Magic](/help/quick-add-magic).

By default, done tasks are hidden in the list view. You can change that by editing the view filter.

### Sorting the list

<Screenshot src="views-list-sort-popup" alt="List view sort popup open with sort options" />
...
The list view sorts by one field at a time. Your choice is stored for that view and written to the URL, so you can bookmark a list with a specific sort order or share the link with someone else.

Sorting was added in Vikunja **2.3.0**. In the [table view](#table-view), you can still sort by clicking column headers directly, and hold `Ctrl` or `Cmd` to sort by more than one column at once.

## Gantt view

<Screenshot src="views-gantt" alt="Gantt view showing tasks on a timeline" />

The Gantt view places tasks on a timeline using their [start and end dates](/help/dates-and-reminders). Use it when scheduling matters more than a simple list.
...
- **Dependency arrows:** [Relations](/help/task-relations) between tasks are shown as arrows on the chart. The arrows update in real time when you drag or resize a task.
...
<Screenshot src="views-table" alt="Table view showing tasks in a spreadsheet-like layout" />
...
- **Sorting:** Click a column header to sort by it. Hold `Ctrl` or `Cmd` while clicking to sort by multiple columns. The sort order is remembered for this view.

Use table view when you want to compare many task fields side by side.

## Kanban view

<Screenshot src="views-kanban" alt="Kanban view with tasks organized into buckets" />
...
<Screenshot src="views-kanban-bucket-menu" alt="Kanban bucket three-dot menu with options" />
...
<Screenshot src="views-kanban-wip-limit" alt="Kanban bucket with a WIP limit indicator" />
...
The bucket header shows the current task count alongside the limit (for example, "5/10"). You can enable an option in your [user settings](/help/settings) to always show the task count on buckets, even when no limit is set.
...
1. Upload an image as an attachment on the task.
2. Click the icon on the attachment to set it as the cover image.
...
- **[Saved Filters](/help/saved-filters)** are personal, cross-project filter queries. They appear in your sidebar and let you create custom filtered lists across all your projects using the [filter syntax](/help/filters). Saved Filters cannot be shared with other users.

See [Saved Filters](/help/saved-filters) for full details on creating and managing them.
