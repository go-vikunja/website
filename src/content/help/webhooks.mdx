...
slug: webhooks
description: >-
  Send automatic notifications to external services when tasks are created,
...
![Webhooks section in project settings](../../assets/images/help/webhooks-settings-section.png)

1. Open the [project](/help/projects) where you want the webhook.
2. Go to the project settings and select **Webhooks**.
![Webhook creation form with URL, events, and secret fields](../../assets/images/help/webhooks-create-form.png)

3. Fill in the form:
   - **Target URL** (required): The URL that Vikunja will send data to (must start with `http://` or `https://`).
   - **Events** (required): Choose which events should trigger the webhook.
   - **Secret** (optional): A shared secret for verifying that requests really come from Vikunja. See [Signing](#signing) below.
   - **Basic Auth** (optional): Click **Use Basic Auth?** to enter a username and password if the target URL requires it.
4. Click **Save**.

### User webhooks

1. Open your **[Settings](/help/settings)** and go to the **Webhooks** section.
2. Fill in the same form as above. User webhooks only allow user-directed events (reminders and overdue notifications).
...
If you set a **secret** when creating the webhook, Vikunja includes an `X-Vikunja-Signature` header in every request. This header contains an HMAC-SHA256 signature of the request body, which lets the receiving service verify that the request genuinely came from Vikunja.

See [webhooks.fyi](https://webhooks.fyi/security/hmac) for details on how to validate HMAC signatures.
...
- `event_name`: Which event triggered the webhook.
- `time`: When the webhook was sent, in ISO 8601 format.
- `data`: The event data, containing the relevant objects (task, project, user, etc.) depending on the event type.

## API reference

To create and manage webhooks programmatically, see the [Webhooks API Reference](/docs/webhooks). You can create [API tokens](/help/settings#api-tokens) in your settings to authenticate with the API.
...
- Instance administrators can configure webhook timeouts and proxy settings. See the [webhook configuration options](/docs/config-options#0--webhooks) for details.
