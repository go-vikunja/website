...
slug: dates-and-reminders
description: Due dates, start and end dates, reminders, and repeating tasks.
---
...
<Screenshot src="dates-overdue-highlight" alt="Overdue task highlighted in the task list" />

If you've enabled it in the settings, you'll receive an email each morning with a list of the tasks that are overdue.

Due dates are also used for sorting in the **Upcoming** view and on the **[dashboard](/help/dashboard)**, making it easy to see what is coming up next.

### Default due time

If you usually make tasks due at the same time of day, set a **Default due time** under **Settings > General > Projects & Tasks**. Vikunja uses it when you choose a date without a time, including dates added with [Quick Add Magic](/help/quick-add-magic).

Times you enter explicitly are left unchanged. If you leave the setting empty, Vikunja picks its usual nearest time of day.
...
These dates are primarily used in the **[Gantt](/help/views#gantt-view)** view, where each task appears as a bar spanning from its start date to its end date on the timeline.

## Reminders

<Screenshot src="dates-reminder-config" alt="Reminder configuration with absolute and relative options" />

You can set one or more reminders on any task. When a reminder triggers, you receive a [notification](/help/search-and-navigation#notifications) in the Vikunja UI. If you have email notifications enabled in your [settings](/help/settings), you will also receive an email.
...
<Screenshot src="dates-repeating-task" alt="Repeating task configuration" />
...
For the full list of supported keywords and date formats, see [Quick Add Magic](/help/quick-add-magic).
