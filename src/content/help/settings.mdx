...
slug: settings
description: >-
  All user settings: general preferences, password, two-factor authentication,
  email, avatar, data export, import, CalDAV, MCP, API tokens, and account deletion.
---
...
- **Two-factor authentication:** protect a local account with a code from an authenticator app as well as your password.
...
Choose the time Vikunja uses when you set a [due date](/help/dates-and-reminders#due-date) without specifying a time in date-picker selections and dates parsed by [Quick Add Magic](/help/quick-add-magic). An explicitly entered time always takes precedence.
...
From Vikunja **2.6.0** onwards, a new email address does not take effect until you confirm it. After you request the change:

- Your old address stays active. You keep receiving notifications there, and you keep signing in with it.
- The new address gets a confirmation mail. Click the link in it to complete the change.
- Your old address gets a mail telling it a change was requested, so you find out if it wasn't you.

The settings page shows the pending address while it waits for confirmation, with buttons to **resend** the confirmation mail or **cancel** the change. Resending is rate limited, so you may have to wait a moment between attempts.

If your instance has no mailer configured, there is nothing to confirm and the new address applies immediately.
...
Local accounts can require a six-digit code from an authenticator app in addition to the account password. See [Two-factor authentication](/help/two-factor-authentication) for setup, sign-in, CalDAV, and recovery details.
...
Connect AI clients to your tasks and projects. This tab shows your MCP endpoint, creates tokens with the permissions you choose, and provides client setup instructions.

See [MCP](/help/mcp/) for supported clients, setup steps, and how to revoke access.
...
For more information on using the API, see the [API documentation](/docs/api-documentation).
