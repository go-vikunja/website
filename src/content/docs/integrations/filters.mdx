...
slug: filters
description: >-
  Technical reference for using Vikunja's filter syntax through the API,
  including field names, query parameters, and date math.
---

This page covers the technical details of using Vikunja's filter syntax through the API. For an introduction to filters and how to use them in the web UI, see the [Filters help page](/help/filters).
...
The `created_by` field matches usernames, not numeric user IDs:

```
created_by = 'alice'
created_by in 'alice,bob'
...
The syntax is similar to the date math used by [Grafana](https://grafana.com/docs/grafana/latest/dashboards/time-range-controls/) and [Elasticsearch](https://www.elastic.co/guide/en/elasticsearch/reference/7.3/common-options.html#date-math).
