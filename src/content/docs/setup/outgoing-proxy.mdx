...
Requests that go through the proxy are resolved by the proxy, so it is the proxy's job to block requests to internal services. Requests that bypass the proxy, such as hosts listed in `NO_PROXY`, are still checked by Vikunja.
