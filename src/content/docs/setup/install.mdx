...
Replace `stable` with `unstable` to track builds from the main development branch. The `unstable` channel is available now; `stable` packages are published starting with Vikunja 2.4.0.
...
Replace `stable` with `unstable` in the `baseurl` to track builds from the main development branch. The `unstable` channel is available now; `stable` packages are published starting with Vikunja 2.4.0.
...
```sh
sudo pacman-key --recv-keys FF054DACD908493A
...
wget -O /etc/apk/keys/vikunja.rsa.pub https://dl.vikunja.io/repos/apk/key.rsa.pub
