...
To be able to upload task attachments or change the background of a project, Vikunja must be able to write into the `files` directory.
...
If you're using [rootless Docker](/docs/full-docker-example#rootless-docker), see the dedicated section about file permissions in that setup.
...
<p>**There is no shell inside the container.** Commands like `docker exec -it vikunja sh` will not work. Use `docker logs vikunja` to check for errors instead.</p>
<p>**`PUID` and `PGID` environment variables are not supported.** These are conventions from other images (like LinuxServer.io) but have no effect on the Vikunja container. To change the user the container runs as, use Docker's [`user:` directive](https://docs.docker.com/compose/how-tos/user/) in your compose file.</p>
<p>**The `/tmp` directory must be writable** for data exports to work. If you use a custom user, ensure `/tmp` is accessible or mount a writable volume at `/tmp`.</p>
</Callout>
...
Especially when using PostgreSQL, this can happen on first start, because the PostgreSQL database container will take a few seconds to start.
...
This means the `files` directory is not writable by the Vikunja process. Make sure you followed the [file permission steps above](#ensure-adequate-file-permissions).
...
- Not using the `-R` flag when running `chown` on a directory that already contains files: `chown -R 1000 $PWD/files`
...
This is a [known Go runtime warning](https://github.com/golang/go/issues/75505) and is harmless. Vikunja will work correctly despite this message.
