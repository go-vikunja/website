...
<p>**`PUID` and `PGID` environment variables are not supported.** These are conventions from other images (like LinuxServer.io) but have no effect on the Vikunja container. To change the user the container runs as, use Docker's [`user:` directive](https://docs.docker.com/reference/compose-file/services/#user) in your compose file.</p>
