...
- **Yaegi** (recommended): Loads plugins from Go source files. No compilation step needed — just drop `.go` source files into a directory and Vikunja interprets them at runtime using [Yaegi](https://github.com/traefik/yaegi). Works on all platforms including Windows.
- **Native** (deprecated): Loads compiled Go shared libraries (`.so` files). Requires exact Go version and dependency matching. Only works on Linux and macOS.
...
Each plugin is a subdirectory containing `.go` source files:
...
Each plugin is a compiled `.so` file:
...
**Yaegi plugins:**
1. Copy the plugin source directory to your configured plugins directory
2. Restart Vikunja
3. Verify in the logs if the plugin has been loaded successfully
...
1. Stop Vikunja
2. Delete the plugin directory (yaegi) or `.so` file (native) from the plugins directory
3. Restart Vikunja
...
Plugins can add new API endpoints. Authenticated routes are available under `/api/v1/plugins/`, unauthenticated routes under `/plugins/`.
...
2. **Yaegi: Single-file plugins only (for now)**: The yaegi loader evaluates `.go` files individually, so multi-file plugins may hit order-dependency issues
3. **Yaegi: Interpreted performance**: Yaegi plugins run interpreted, not compiled. This is fine for hooks and route handlers but would be a concern for hot-path code
4. **Native: Version Sensitivity**: Exact Go version match required between plugin and Vikunja
5. **Native: Platform Specific**: Native plugins only work on Linux and macOS
6. **Memory Sharing**: Plugins share the same memory space as Vikunja
