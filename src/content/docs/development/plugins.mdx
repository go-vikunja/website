...
- **Yaegi** (recommended): Interprets Go source files at runtime using [Yaegi](https://github.com/traefik/yaegi). No compilation step needed.
- **Native** (deprecated): Uses [Go's native plugin system](https://pkg.go.dev/plugin) to load compiled `.so` shared libraries.
...
Create a directory for your plugin with a `main.go` file:
...
Only `NewPlugin()` is required. The other factory functions are optional and discovered by the loader. If a factory function is not exported, the corresponding capability is simply not registered.
...
Authenticated routes are prefixed with `/api/v1/plugins/`, unauthenticated routes with `/plugins/`.
...
- **`yaegi`**: scans `plugins.dir` for subdirectories containing `.go` source files. Plugins are interpreted at runtime.
- **`native`** (deprecated): scans `plugins.dir` for `.so` shared library files. Plugins must be compiled with the exact same Go version and dependencies as the Vikunja binary.
...
**Yaegi limitation**: The yaegi loader currently evaluates `.go` files individually, so multi-file plugins may hit order-dependency issues if cross-file declarations depend on filename ordering. For now, keeping your plugin in a single `main.go` file is the safest approach.
</Callout>
...
See [`examples/plugins/example/`](https://code.vikunja.io/vikunja/tree/main/examples/plugins/example) for a full working plugin that demonstrates:
