...
A valid TOTP code could be reused within its 30-second window to authenticate multiple sessions. Note: the fix for this issue was not merged in time for 2.2.0 and actually shipped in [2.2.1](/changelog/vikunja-v2.2.2-was-released).
...
Unauthenticated rate limits could be bypassed by spoofing `X-Forwarded-For` or `X-Real-IP` headers. The fix adds `service.ipextractionmethod` and `service.trustedproxies` config options to control how the client IP is determined. Check out [the docs](/docs/config-options#1-service-ipextractionmethod) for more info.
...
Webhooks now include built-in SSRF protection to prevent requests to internal/non-routable IP addresses. If you need to allow webhooks to reach local services, you can set `webhooks.allownonroutableips` in your config. Check out [the docs](/docs/config-options#0--webhooks) for more info.
