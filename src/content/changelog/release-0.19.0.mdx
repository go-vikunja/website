...
description: >-
  Vikunja 0.19.0: Dark mode, mobile app updates,
  task reminders, and CalDAV tokens.
---
...
[Vikunja Cloud](https://app.vikunja.cloud/?redirectToProvider=true) now finally makes it easy to sign up your whole team and centrally manage billing.
...
Most notably, it can now send you reminders as push notifications.
...
This is different from the [`service.timezone`](https://vikunja.io/docs/config-options#1-service-timezone) configuration option for the server.

Once configured, Vikunja will use this setting to send you email reminders at the time you set them up in your time zone.
With this addition, email reminders now work a lot more intuitively than before.
...
They consist of special tokens for Day, Week, Month, etc. and let you add or subtract amounts relative to each other.
...
* Rate limits for unauthenticated routes: Routes which do not require authentication are now also covered by the rate limit, [if enabled and configured](https://vikunja.io/docs/config-options#0--ratelimit).
