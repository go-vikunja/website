---
title: Vikunja 0.14.1 is released
date: 2020-07-07T12:00:00.000Z
isRelease: true
slug: /vikunja-0.14.1-is-released
description: >-
  Bugfix release 0.14.1 for Vikunja API with updates for list creation and
  user management.
---

This is a smaller bugfix release with some improvements for the api.

As always, you can grab a release from [the downloads page](https://dl.vikunja.io/api/0.14.1) or [docker hub](https://hub.docker.com/r/vikunja/api).

### Fixed

* Fix creating lists with non ascii characters (#607)
* Fix decoding active users from redis
* Fix parsing todoist reminder dates
* Make sure the metrics map accesses only happen explicitly

### Changed

* Update docs theme
